...
describe("Interact with input fields", () => {
    ...
    test("Enter data on text box page", async () => {
        ...
        expect(await page.innerText(label_currentAddress)).toBe(
            "Current Address"
        );
        expect(await page.innerText(label_PermanentAddress)).toBe(
            "Permanent Address"
        );
        ...
        expect(await page.innerText(currentAddressOutput_xpath)).toContain(
            currentAddress_data
        );
        expect(await page.innerText(permanentAddressOutput_xpath)).toContain(
            permanentAddress_data
        );
    });
